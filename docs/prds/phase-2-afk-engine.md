...
- [ ] Under `governance.product: delegated`, a Full-lane item's approval is provable from the journal: the second vendor's independent verification, the append-only decision record naming both vendors, and the status flip under delegated authority — and each park trigger works: vendor disagreement parks, a constitution conflict parks, and a `seed` inception tier parks regardless of config (F2.2).
