...
`with_timeout` comes from `scripts/lib/with-timeout.sh`, sourced by every script that bounds a child process (`codex-scope.sh`, the three review scripts, `test-current.sh`). It delegates to GNU `timeout`/`gtimeout` when installed and otherwise falls back to a perl watchdog — stock macOS ships no coreutils, so a bare `timeout` call dies with 127 there. Both paths behave identically: the child runs in its own process group, expiry TERMs that group and escalates to KILL after a 2s grace (so a child that traps or ignores TERM still hits a hard cap, and no descendant outlives the timeout), and the exit code is 124 — which is what the status-file branches key on.
