...
- **F9.4** **Founding mode is an explicit up-front choice** (Jim-directed 2026-07-25): the inception workflow's first question is "grill session (guided), or give me a paragraph and I figure it out (hands-off)?" — and as the shortcut form, `nahel init --hands-off "<paragraph>"` records the mode and the paragraph as ordinary deterministic state (the CLI stores the choice; the inception workflow acts on it). Both doors per hard constraint 5; naming of the flag is cosmetic and swappable.
- **F9.5** **Hands-off founding** needs no return visit, and stays constitutional: the human's paragraph is preserved **verbatim as the signed core of the constitution** — the journaled, human-attributed init/kickoff act is the signature. The agent's knowledge-first elaboration around it is verified by the second vendor (the same cross-vendor consensus as F2.2) before the founding records `standard`; everything added beyond the paragraph is journaled as assumptions; `governance.product` records `delegated`. Any later work that would **contradict the signed paragraph parks** — the one human instruction is the one thing the run can never override.
...
- [ ] A hands-off founding from a paragraph reaches founded state with **zero human turns after the init/kickoff act**: the paragraph appears verbatim as the constitution's signed core with human-attributed journal provenance, the elaboration carries the cross-vendor verification event and the assumption trail, and `governance.product` records `delegated` (F9.4/F9.5).
- [ ] A work item whose direction contradicts the signed paragraph parks rather than proceeding — exercised once in the lab with a deliberately contradictory drafted PRD (F9.5).
