...
`with_timeout` comes from `scripts/lib/with-timeout.sh`, sourced by every script that bounds a child process (`codex-scope.sh`, the three review scripts, `test-current.sh`). It delegates to GNU `timeout`/`gtimeout` when installed and otherwise falls back to a perl watchdog — stock macOS ships no coreutils, so a bare `timeout` call dies with 127 there. Either path exits 124 on expiry, which is what the status-file branches key on.
