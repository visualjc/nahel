...
#     descendant outlives the timeout (this is also GNU timeout's default)
...
# Both paths get the same TERM->KILL grace, so they promise the same thing.
#
...
with_timeout() {
  ...
  if command -v timeout >/dev/null 2>&1; then
    timeout -k "$WITH_TIMEOUT_KILL_AFTER" "$seconds" "$@"
  elif command -v gtimeout >/dev/null 2>&1; then
    gtimeout -k "$WITH_TIMEOUT_KILL_AFTER" "$seconds" "$@"
  else
    perl -e '
      use POSIX ();
      ...
      $SIG{ALRM} = sub {
        kill "TERM", -$pid;
        # Poll rather than blind-sleep the grace: a child that honours TERM
        # must not cost every timeout an extra $kill_after seconds.
