...
4. Verify dependencies in this order (in the RESOLVED repo), halt with clear error on any failure:
   ...
   - CCPM context bundle present (`ls .claude/context/*.md`) → informational only; if missing, log to `progress.md`: "no .claude/context/*.md present; codex priming will be skipped this run" and continue. Not a halt — repos without `/context:create` ever run still work.
