...
- **Phase 5 PRD review, Phase 7 epic review, Phase 10.2.e diff review** — `gpt-5.3-codex-spark` with `--ignore-user-config --sandbox read-only --json` under `with_timeout 600`. Fast review-only model; bounded, deterministic, no project-rule overrides bleeding into the review pass.
...
`with_timeout` comes from `scripts/lib/with-timeout.sh`, sourced by every script that bounds a child process (`codex-scope.sh`, the three review scripts, `test-current.sh`). It is a perl watchdog, used uniformly and never delegating to GNU `timeout` even where that exists: stock macOS ships no coreutils (a bare `timeout` call dies with 127 there), and `timeout -k` reports 137 rather than 124 when the KILL escalation is what ended the command — indistinguishable from an external kill. The child runs in its own process group; expiry TERMs that group, escalates to KILL after a 2s grace, and does not return until the group is empty — so a child that traps or ignores TERM still hits a hard cap, and no descendant outlives the timeout even when the direct child exits cleanly. The exit code is 124, which is what the status-file branches key on. perl is therefore required — without it `with_timeout` refuses with 125 rather than running the command unbounded.
